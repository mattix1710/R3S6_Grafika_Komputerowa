...
function updatePointCloud(vertexPosition,vertexVelocity,vertexColor,vertexAge,n,dt, sx,ex,sy,ey,sz,ez,svx,evx,svy,evy,svz,evz,sa,ea)
{
  for(let i=0;i<n;i++)
  {
    vertexPosition[i*3+0] = vertexPosition[i*3+0] + vertexVelocity[i*3+0]*dt/2;             //less movement to the sides (left-right)
    vertexPosition[i*3+1] = Math.abs(vertexPosition[i*3+1] + vertexVelocity[i*3+1]*dt*2);   //more movement to UP
    vertexPosition[i*3+2] = vertexPosition[i*3+2] + vertexVelocity[i*3+2]*dt/2;             //less movement to the sides (front-back)
...
async function startGL() 
{
  ...
  ///////////////////////////////////////////////////
  // drugi SHADER PROGRAM - kłody
  //

  //Kod shaderów
  const vertextShaderSource2 = ` //Znak akcentu z przycisku tyldy - na lewo od przycisku 1 na klawiaturze
    precision highp float;
    attribute vec3 aVertexPosition; 
    attribute vec3 aVertexNormal;
    uniform mat4 uMMatrix;
  ...
  gl.shaderSource(fragmentShader2, fragmentShaderSource2); //Podpięcie źródła kodu shader
  gl.shaderSource(vertexShader2, vertextShaderSource2);
  gl.compileShader(fragmentShader2); //Kompilacja kodu shader
  gl.compileShader(vertexShader2);
  if (!gl.getShaderParameter(fragmentShader2, gl.COMPILE_STATUS)) { //Sprawdzenie ewentualnych błedów kompilacji
  ...
  shaderProgram2 = gl.createProgram(); //Stworzenie obiektu programu 
  gl.attachShader(shaderProgram2, vertexShader2); //Podpięcie obu shaderów do naszego programu wykonywanego na karcie graficznej
  gl.attachShader(shaderProgram2, fragmentShader2);
  gl.linkProgram(shaderProgram2);
  if (!gl.getProgramParameter(shaderProgram2, gl.LINK_STATUS)) alert("Could not initialise shaders");  //Sprawdzenie ewentualnych błedów
  ...
  [indexes2, vertexPosition2, vertexNormal2] = await LoadObj('./fireplaceSrc/fireplace.obj');
  ...
  //[vertexPosition, vertexColor, vertexCoords, vertexNormal] = CreateShpere(0,0,0,2, 6, 12); 
  [vertexPositionParticle, vertexVelocityParticle, vertexColorParticle, vertexAge] = CreatePointCloud(sx,ex,sy,ey,sz,ez,svx,evx,svy,evy,svz,evz,sa,ea,1000);
...
{ 
  ...
  let uMMatrix0_2 = CreateIdentytyMatrix(); 
  ...
  uMMatrix0_2 = MatrixMul(uMMatrix0_2, CreateScaleMatrix(fireplaceSize, fireplaceSize, fireplaceSize));
  uMMatrix0_2 = MatrixMul(uMMatrix0_2, CreateTranslationMatrix(LightPosX2, LightPosY2, LightPosZ2));
  ...
  gl.clearColor(0.0,0.0,0.0,1.0); //Wyczyszczenie obrazu kolorem czerwonym            //INFO: background color
  ...
  // new here
  //
  // SUPER PRO-TIP!!!!!! <-- for new objects, create new "program" - here it is shaderProgram2
  //
  ...
  gl.uniformMatrix4fv(gl.getUniformLocation(shaderProgram2, "uVMatrix"), false, new Float32Array(uVMatrix));
  gl.uniform1f(gl.getUniformLocation(shaderProgram2, "uNormalMul"),1.0);
  gl.uniform3f(gl.getUniformLocation(shaderProgram2, "uLightPosition"),LightPositionX,LightPositionY,LightPositionZ);
  
  gl.enableVertexAttribArray(gl.getAttribLocation(shaderProgram2, "aVertexPosition"));  //Przekazanie położenia
  gl.bindBuffer(gl.ARRAY_BUFFER, vertexPositionBuffer2);
  gl.vertexAttribPointer(gl.getAttribLocation(shaderProgram2, "aVertexPosition"), vertexPositionBuffer2.itemSize, gl.FLOAT, false, 0, 0);
  
  gl.enableVertexAttribArray(gl.getAttribLocation(shaderProgram2, "aVertexNormal"));  //Przekazywanie wektorów normalnych
  gl.bindBuffer(gl.ARRAY_BUFFER, vertexNormalBuffer2);
  gl.vertexAttribPointer(gl.getAttribLocation(shaderProgram2, "aVertexNormal"), vertexNormalBuffer2.itemSize, gl.FLOAT, false, 0, 0);
  
  gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, indexBuffer2);

  //Obiekt Światła - jak nie ma gl.uniform1f to traktowany jako zwykły obiekt, działa mi światło z pierwszego "shaderProgram"
  gl.uniformMatrix4fv(gl.getUniformLocation(shaderProgram2, "uMMatrix"), false, new Float32Array(uMMatrix0_2));
  gl.uniformMatrix4fv(gl.getUniformLocation(shaderProgram2, "uInvMMatrix"), false, new Float32Array(MatrixTransposeInverse(uMMatrix0_2)));
  gl.uniform3f(gl.getUniformLocation(shaderProgram2, "uColor"),0.39,0.26,0.13);
  //gl.uniform1f(gl.getUniformLocation(shaderProgram2, "uNormalMul"),-1.0);  
  //gl.drawArrays(gl.TRIANGLES, 0, vertexPositionBuffer.numItems*vertexPositionBuffer.itemSize); //Faktyczne wywołanie rendrowania
  ...
  [vertexPositionParticle, vertexVelocityParticle, vertexColorParticle, vertexAge] = updatePointCloud(vertexPositionParticle,vertexVelocityParticle,vertexColorParticle,vertexAge,1000,0.01, sx,ex,sy,ey,sz,ez,svx,evx,svy,evy,svz,evz,sa,ea);
